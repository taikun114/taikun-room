/* eslint-disable */
      /* This is a auto generated file for building the project */ 


      import { Fragment, useState } from "react";
      import { useResource, useVariableState } from "@webstudio-is/react-sdk/runtime";
      import { Body as Body, Link as Link, RemixForm as RemixForm } from "@webstudio-is/sdk-components-react-router";
import { Slot as Slot, Image as Image, Fragment as Fragment_1, Box as Box, Heading as Heading, Text as Text, HtmlEmbed as HtmlEmbed, Time as Time, Input as Input, Button as Button } from "@webstudio-is/sdk-components-react";


      export const projectId = "dba9d753-ddf9-434f-be13-fdc5fabadb2d";

      export const lastPublished = "2025-08-26T11:56:07.029Z";

      export const siteName = "たいくんの部屋";

      export const breakpoints = [{"id":"7hPMMapQ_YKRAmEWw_M-S"},{"id":"oIVfj9CW1VSGQ1nzAMQhV","maxWidth":991},{"id":"b-tdPHJg4N8hzh8wxGSqL","maxWidth":767},{"id":"sURTnOO_xRAsuAvyLcB6L","maxWidth":479}];

      export const favIconAsset: string | undefined =
        "favicon_KlZhqsaIZDngBH2QBAuUy.png";

      // Font assets on current page (can be preloaded)
      export const pageFontAssets: string[] =
        []

      export const pageBackgroundImageAssets: string[] =
        []

      
            
            const Script = ({children, ...props}: Record<string, string | boolean>) => {
              if (children == null) {
                return <script {...props} />;
              }

              return <script {...props} dangerouslySetInnerHTML={{__html: children}} />;
            };
            const Style = ({children, ...props}: Record<string, string | boolean>) => {
              if (children == null) {
                return <style {...props} />;
              }

              return <style {...props} dangerouslySetInnerHTML={{__html: children}} />;
            };
            

            export const CustomCode = () => {
              return (<><Style>{"\n  @media (prefers-color-scheme: dark) {\n    img {\n      filter: brightness(90%);\n    }\n  }\n"}</Style>{"\n\n"}<Style>{"\n@import url('https://fonts.googleapis.com/css2?family=Inter:ital,opsz,wght@0,14..32,100..900;1,14..32,100..900&family=Noto+Sans+JP:wght@100..900&display=swap');\n"}</Style>{"\n\n"}<Script async src={"https://www.googletagmanager.com/gtag/js?id=G-K4H191M0HY"}></Script><Script>{"\n  window.dataLayer = window.dataLayer || [];\n  function gtag(){dataLayer.push(arguments);}\n  gtag('js', new Date());\n\n  gtag('config', 'G-K4H191M0HY');\n"}</Script>{"\n\n"}<Script async src={"https://pagead2.googlesyndication.com/pagead/js/adsbygoogle.js?client=ca-pub-4148785556928544"} crossorigin={"anonymous"}></Script>{"\n\n"}<Script defer src={"https://umami.taikun.blog/script.js"} data-website-id={"476ec6fa-4fdc-4711-afff-e403e5c5e57c"}></Script></>);
            }
          

      const Page = (_props: { system: any; }) => {
let _ = useResource("__1")
return <Body
className={`w-body`}>
<Slot>
<Fragment_1>
<Image
src={"/assets/alex-shuper-easvYe4ixJk-unsplash_zQPZasAHcYDYOcNBhnIxr.webp"}
width={1280}
height={720}
loading={"eager"}
className={`w-image ckw650q cu39xf9 c1ysgjff cok98yc`} />
<Box
className={`w-box ckw650q cu39xf9 cok98yc c1epj6oz cgzp0rh`} />
</Fragment_1>
</Slot>
<Box
className={`w-box c16u96pk c1nrl0mm cv98wn5 ckw650q clrs9cu clb7ee7 c1ayo3bo`}>
<Box
className={`w-box ckw650q c6g550q c16u96pk c1nrl0mm cv98wn5 clb7ee7 c1y3dgs3 cvcpz3a ci80ji7 cfl86vg`}>
<Link
href={"/"}
className={`w-link ckw650q c30d1di`}>
<Image
src={"/assets/taikun-room-full_SXGNYnXHAdTh7JkRN0ZcG.png"}
width={1200}
height={129}
loading={"eager"}
alt={"TAIKUN Room.com | たいくんの部屋"}
fetchPriority={"high"}
className={`w-image`} />
</Link>
<Box
data-ws-tag="header"
className={`w-box ckw650q c30d1di`}>
<Box
className={`w-box c1mo0hrg c9ztfc4 cvpjo14 c1ivbbvm c6cj56m ctiy6wq c1yz7033 c16u96pk cckrrlb c1nrl0mm ct6b9qv c1demw6w ci2ryas c1mp8u76 c1sy5cbg ctibe6j cq98dcv c1nljn1v c1bk6428 cx7dkne c1rhsvz0`}>
<Box
className={`w-box c16u96pk cxvivdm cmqkj62 ct6b9qv`}>
<Link
href={"/"}
className={`w-link c14njgcc c9ll3mf cxhsvi csiqldl cywehne c1w3kw24 ckfbvdv cnny46r c1x20bz7 c1kiu74r cu0zbn6 cgzd7q6 cqs822p c1t4den9 cwlahma cb1oezn c1bk6428 c1innfc3 cuklm3e clnhelo`}>
{"ホーム"}
</Link>
<Link
href={"/page/notice"}
className={`w-link c14njgcc c9ll3mf cxhsvi csiqldl cywehne c1w3kw24 ckfbvdv cnny46r c1x20bz7 c1kiu74r cu0zbn6 cgzd7q6 cqs822p c1t4den9 cwlahma c1bk6428 c1innfc3 cuklm3e clnhelo`}>
{"お知らせ"}
</Link>
<Link
href={"/page/contact"}
className={`w-link c14njgcc c9ll3mf cxhsvi csiqldl cywehne c1w3kw24 ckfbvdv cnny46r c1x20bz7 c1kiu74r cu0zbn6 cgzd7q6 cqs822p c1t4den9 cwlahma c1bk6428 c1innfc3 cuklm3e clnhelo`}>
{"お問い合わせ"}
</Link>
<Link
href={"/page/privacy-policy"}
className={`w-link c14njgcc c9ll3mf cxhsvi csiqldl cywehne c1w3kw24 ckfbvdv cnny46r c1x20bz7 c1kiu74r cu0zbn6 cgzd7q6 cqs822p c1t4den9 cwlahma c1bk6428 c1innfc3 cuklm3e clnhelo`}>
{"プライバシーポリシー"}
</Link>
<Link
href={"https://www.taikun-life.com/"}
className={`w-link c14njgcc c9ll3mf cxhsvi csiqldl cywehne c1w3kw24 ckfbvdv cnny46r c1x20bz7 c1kiu74r cu0zbn6 cgzd7q6 cqs822p c1t4den9 cwlahma c1bk6428 c1innfc3 ci10kzm clnhelo`}>
{"たいくんの生活"}
</Link>
<Link
href={"https://shop.taikun.design/"}
className={`w-link c14njgcc c9ll3mf cxhsvi csiqldl cywehne c1w3kw24 ckfbvdv cnny46r c1x20bz7 c1kiu74r cu0zbn6 cgzd7q6 cqs822p c1t4den9 cwlahma c1bk6428 c1innfc3 cuklm3e clnhelo`}>
{"たいくんのお店"}
</Link>
</Box>
<RemixForm
className={`w-form c16u96pk ckfbvdv cnny46r c1x20bz7 c1kiu74r c88e3vu ctzx07w`}>
<Input
aria-label={"検索ボックス"}
name={"search"}
placeholder={"検索..."}
type={"search"}
className={`w-text-input c18xfrfr ccpbbov c4oa98r clun22x c14eq3al c104iyh7 c1iftn02 c17iwvim c16tp3ap ckfbvdv c1x20bz7 c9ll3mf c1dgsltx ctzx07w`} />
<Button
aria-label={"検索ボタン"}
className={`w-button c15p346f c18xfrfr ccpbbov c4oa98r clun22x coi25rp cnny46r c1kiu74r`}>
<Box
className={`w-box cpspxpc c1d3h5bc chpom5m c329hqk c1sy5cbg cxovp9q c2y4chl c1nljn1v`}>
<HtmlEmbed
code={"<svg xmlns=\"http://www.w3.org/2000/svg\" viewBox=\"0 0 512 512\"><!--!Font Awesome Free 6.7.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free Copyright 2025 Fonticons, Inc.--><path fill=\"#ffffff\" d=\"M416 208c0 45.9-14.9 88.3-40 122.7L502.6 457.4c12.5 12.5 12.5 32.8 0 45.3s-32.8 12.5-45.3 0L330.7 376c-34.4 25.2-76.8 40-122.7 40C93.1 416 0 322.9 0 208S93.1 0 208 0S416 93.1 416 208zM208 352a144 144 0 1 0 0-288 144 144 0 1 0 0 288z\"/></svg>"}
className={`w-html-embed c16u96pk c2en5ep c1uczmal cu39xf9 ckw650q`} />
</Box>
</Button>
</RemixForm>
</Box>
</Box>
<Box
className={`w-box ckw650q c16u96pk ct6b9qv c1demw6w ci2ryas`}>
{_?.data?.data?.posts?.edges?.map?.((collectionItem: any, index: number) =>
<Fragment key={index}>
<Box
data-ws-tag="article"
className={`w-box c1mo0hrg c1yz7033 ctiy6wq c9ztfc4 cvpjo14 c1ivbbvm c6cj56m cfcch3v c1h2qy8m cgzd7q6 cqs822p c1t4den9 cwlahma c1mqbasd c1rhsvz0 corpmch c1f9m8pb`}>
<Link
href={"/blog/" + collectionItem?.node?.slug}
title={collectionItem?.node?.title}
className={`w-link cu39xf9 ckw650q c14njgcc ct7i05a`}>
<Box
className={`w-box`}>
<Image
src={collectionItem?.node?.featuredImage?.node?.sourceUrl}
loading={"eager"}
width={collectionItem?.node?.featuredImage?.node?.mediaDetails?.width}
height={collectionItem?.node?.featuredImage?.node?.mediaDetails?.height}
alt={collectionItem?.node?.featuredImage?.node?.altText}
className={`w-image c9ztfc4 cvpjo14 ckw650q cpbvj9u c1ysgjff`} />
<Box
className={`w-box clb7ee7 cckrrlb c1kwnyvg`}>
<Heading
className={`w-heading c1xyw2bj cffkjh0 c11z3i3x c9ll3mf cinzvyd cmendya c1do09oa c1ho1hsm ce9x8e9`}>
{collectionItem?.node?.title}
</Heading>
<HtmlEmbed
code={collectionItem?.node?.excerpt}
className={`w-html-embed c9ll3mf cinzvyd c1u7in6p c1do09oa c1ho1hsm ce9x8e9 c1i9877x cfjs3rk cy61qk1 c30d1di`}>
</HtmlEmbed>
<Box
className={`w-box c16u96pk cxvivdm cmqkj62 ct6b9qv`}>
<Box
className={`w-box c16u96pk c1nrl0mm cv98wn5 cafs8o8 c16tp3ap c17iwvim cqs2xd4 ckhpsmk c1yh0sse c1uvhaql c1hjchh6 cgt0251 cd4zzh3 cmz7znk`}>
<Box
className={`w-box cpspxpc c1d3h5bc c19dka4a chpom5m c329hqk c1sy5cbg cxovp9q c2y4chl c1nljn1v`}>
<HtmlEmbed
code={"<svg xmlns=\"http://www.w3.org/2000/svg\" viewBox=\"0 0 512 512\"><!--!Font Awesome Free 6.7.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free Copyright 2025 Fonticons, Inc.--><path fill=\"#008fff\" d=\"M498.1 5.6c10.1 7 15.4 19.1 13.5 31.2l-64 416c-1.5 9.7-7.4 18.2-16 23s-18.9 5.4-28 1.6L284 427.7l-68.5 74.1c-8.9 9.7-22.9 12.9-35.2 8.1S160 493.2 160 480l0-83.6c0-4 1.5-7.8 4.2-10.8L331.8 202.8c5.8-6.3 5.6-16-.4-22s-15.7-6.4-22-.7L106 360.8 17.7 316.6C7.1 311.3 .3 300.7 0 288.9s5.9-22.8 16.1-28.7l448-256c10.7-6.1 23.9-5.5 34 1.4z\"/></svg>"}
className={`w-html-embed`} />
</Box>
<Time
language={"ja"}
country={"JP"}
dateStyle={"short"}
timeStyle={"none"}
dateTime={collectionItem?.node?.date}
className={`w-time cb1oezn c1kmny9s`} />
</Box>
<Box
className={`w-box c16u96pk c1nrl0mm cv98wn5 cafs8o8 c16tp3ap c17iwvim cqs2xd4 ckhpsmk c1yh0sse c1uvhaql c1hjchh6 cgt0251 cd4zzh3 cmz7znk`}>
<Box
className={`w-box cpspxpc c1d3h5bc c19dka4a chpom5m c329hqk c1sy5cbg cxovp9q c2y4chl c1nljn1v`}>
<HtmlEmbed
code={"<svg xmlns=\"http://www.w3.org/2000/svg\" viewBox=\"0 0 512 512\"><!--!Font Awesome Free 6.7.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free Copyright 2025 Fonticons, Inc.--><path fill=\"#008fff\" d=\"M142.9 142.9c-17.5 17.5-30.1 38-37.8 59.8c-5.9 16.7-24.2 25.4-40.8 19.5s-25.4-24.2-19.5-40.8C55.6 150.7 73.2 122 97.6 97.6c87.2-87.2 228.3-87.5 315.8-1L455 55c6.9-6.9 17.2-8.9 26.2-5.2s14.8 12.5 14.8 22.2l0 128c0 13.3-10.7 24-24 24l-8.4 0c0 0 0 0 0 0L344 224c-9.7 0-18.5-5.8-22.2-14.8s-1.7-19.3 5.2-26.2l41.1-41.1c-62.6-61.5-163.1-61.2-225.3 1zM16 312c0-13.3 10.7-24 24-24l7.6 0 .7 0L168 288c9.7 0 18.5 5.8 22.2 14.8s1.7 19.3-5.2 26.2l-41.1 41.1c62.6 61.5 163.1 61.2 225.3-1c17.5-17.5 30.1-38 37.8-59.8c5.9-16.7 24.2-25.4 40.8-19.5s25.4 24.2 19.5 40.8c-10.8 30.6-28.4 59.3-52.9 83.8c-87.2 87.2-228.3 87.5-315.8 1L57 457c-6.9 6.9-17.2 8.9-26.2 5.2S16 449.7 16 440l0-119.6 0-.7 0-7.6z\"/></svg>"}
className={`w-html-embed`} />
</Box>
<Time
dateTime={collectionItem?.node?.modified}
language={"ja"}
country={"JP"}
dateStyle={"short"}
timeStyle={"none"}
className={`w-time cb1oezn c1kmny9s`} />
</Box>
</Box>
</Box>
</Box>
</Link>
</Box>
</Fragment>
)}
</Box>
<Box
className={`w-box c16u96pk c1demw6w cmqkj62 c16uhwia ct6b9qv c1kdz7t6`}>
{(_?.data?.data?.posts?.pageInfo?.hasPreviousPage) &&
<Link
aria-current={"false"}
aria-label={"最初に戻る"}
href={""}
className={`w-link c14njgcc`}>
<Box
className={`w-box c16u96pk c1nrl0mm cv98wn5 cj7fjzz c1yk9n7d c1fexnx0 cz9jktr cxvivdm cmqkj62 c15p346f ckfbvdv cnny46r c1x20bz7 c1kiu74r cjvdby0 c11v1l1o c1sy5cbg ctibe6j cq98dcv c1nljn1v cwnqwob`}>
<Box
className={`w-box cpspxpc c1d3h5bc chpom5m c329hqk c1sy5cbg cxovp9q c2y4chl c1nljn1v`}>
<HtmlEmbed
code={"<svg xmlns=\"http://www.w3.org/2000/svg\" viewBox=\"0 0 512 512\"><!--!Font Awesome Free 6.7.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free Copyright 2025 Fonticons, Inc.--><path fill=\"#ffffff\" d=\"M41.4 233.4c-12.5 12.5-12.5 32.8 0 45.3l160 160c12.5 12.5 32.8 12.5 45.3 0s12.5-32.8 0-45.3L109.3 256 246.6 118.6c12.5-12.5 12.5-32.8 0-45.3s-32.8-12.5-45.3 0l-160 160zm352-160l-160 160c-12.5 12.5-12.5 32.8 0 45.3l160 160c12.5 12.5 32.8 12.5 45.3 0s12.5-32.8 0-45.3L301.3 256 438.6 118.6c12.5-12.5 12.5-32.8 0-45.3s-32.8-12.5-45.3 0z\"/></svg>"}
className={`w-html-embed c16u96pk cu39xf9 c2en5ep c1uczmal`} />
</Box>
<Text
className={`w-text cpwwn14 cb1oezn`}>
{"最初に戻る"}
</Text>
</Box>
</Link>
}
{(_?.data?.data?.posts?.pageInfo?.hasNextPage) &&
<Link
aria-current={"false"}
aria-label={"さらに表示"}
href={`?after=${_?.data?.data?.posts?.pageInfo?.endCursor}`}
className={`w-link c14njgcc`}>
<Box
className={`w-box c16u96pk c1nrl0mm cv98wn5 cj7fjzz c1yk9n7d c1fexnx0 cz9jktr cxvivdm cmqkj62 c15p346f ckfbvdv cnny46r c1x20bz7 c1kiu74r cjvdby0 c11v1l1o c1sy5cbg ctibe6j cq98dcv c1nljn1v cwnqwob`}>
<Text
className={`w-text cpwwn14 cb1oezn`}>
{"さらに表示"}
</Text>
<Box
className={`w-box cpspxpc c1d3h5bc chpom5m c329hqk c1sy5cbg cxovp9q c2y4chl c1nljn1v`}>
<HtmlEmbed
code={"<svg xmlns=\"http://www.w3.org/2000/svg\" viewBox=\"0 0 320 512\"><!--!Font Awesome Free 6.7.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free Copyright 2025 Fonticons, Inc.--><path fill=\"#ffffff\" d=\"M310.6 233.4c12.5 12.5 12.5 32.8 0 45.3l-192 192c-12.5 12.5-32.8 12.5-45.3 0s-12.5-32.8 0-45.3L242.7 256 73.4 86.6c-12.5-12.5-12.5-32.8 0-45.3s32.8-12.5 45.3 0l192 192z\"/></svg>"}
className={`w-html-embed c16u96pk cu39xf9 c2en5ep c1uczmal`} />
</Box>
</Box>
</Link>
}
</Box>
<Box
data-ws-tag="footer"
className={`w-box c1mo0hrg c9ztfc4 cvpjo14 c1ivbbvm c6cj56m ctiy6wq c1yz7033 c16u96pk cckrrlb c1nrl0mm ct6b9qv c1demw6w ci2ryas c1mp8u76 c1sy5cbg ctibe6j cq98dcv c1nljn1v ckw650q cfprw0w c17fuod0 c1bk6428 cx7dkne c1rhsvz0`}>
<Link
href={"https://status.taikun.blog/"}
target={"_blank"}
className={`w-link c14njgcc`}>
<Box
className={`w-box c16u96pk c1nrl0mm cv98wn5 cj7fjzz cjft9ro c10hsz cz9jktr ckhpsmk c1yh0sse c15p346f ckfbvdv cnny46r c1x20bz7 c1kiu74r c1omme4v c11v1l1o c1sy5cbg ctibe6j cq98dcv c1nljn1v cwnqwob`}>
<Box
className={`w-box cpspxpc c1d3h5bc c19dka4a chpom5m c329hqk c1sy5cbg cxovp9q c2y4chl c1nljn1v`}>
<HtmlEmbed
code={"<svg xmlns=\"http://www.w3.org/2000/svg\" viewBox=\"0 0 512 512\"><!--!Font Awesome Free 6.7.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free Copyright 2025 Fonticons, Inc.--><path fill=\"#ffffff\" d=\"M64 32C28.7 32 0 60.7 0 96l0 64c0 35.3 28.7 64 64 64l384 0c35.3 0 64-28.7 64-64l0-64c0-35.3-28.7-64-64-64L64 32zm280 72a24 24 0 1 1 0 48 24 24 0 1 1 0-48zm48 24a24 24 0 1 1 48 0 24 24 0 1 1 -48 0zM64 288c-35.3 0-64 28.7-64 64l0 64c0 35.3 28.7 64 64 64l384 0c35.3 0 64-28.7 64-64l0-64c0-35.3-28.7-64-64-64L64 288zm280 72a24 24 0 1 1 0 48 24 24 0 1 1 0-48zm56 24a24 24 0 1 1 48 0 24 24 0 1 1 -48 0z\"/></svg>"}
className={`w-html-embed`} />
</Box>
<Text
className={`w-text cpwwn14 cb1oezn`}>
{"サーバーステータス"}
</Text>
</Box>
</Link>
<Box
className={`w-box c16u96pk cxvivdm cmqkj62 ct6b9qv`}>
<Link
href={"https://x.com/taikun_room_com"}
target={"_blank"}
className={`w-link c14njgcc`}>
<Box
className={`w-box c16u96pk c1nrl0mm cv98wn5 cj7fjzz cjft9ro c10hsz cz9jktr ckhpsmk c1yh0sse c15p346f ckfbvdv cnny46r c1x20bz7 c1kiu74r c1omme4v c11v1l1o c1sy5cbg ctibe6j cq98dcv c1nljn1v cwnqwob`}>
<Box
className={`w-box cpspxpc cw8jop0 c1iiw5pw chpom5m c329hqk c1sy5cbg cxovp9q c2y4chl c1nljn1v`}>
<HtmlEmbed
code={"<svg xmlns=\"http://www.w3.org/2000/svg\" viewBox=\"0 0 512 512\"><!--!Font Awesome Free 6.7.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free Copyright 2025 Fonticons, Inc.--><path fill=\"#fff\" d=\"M389.2 48h70.6L305.6 224.2 487 464H345L233.7 318.6 106.5 464H35.8L200.7 275.5 26.8 48H172.4L272.9 180.9 389.2 48zM364.4 421.8h39.1L151.1 88h-42L364.4 421.8z\"/></svg>"}
className={`w-html-embed`} />
</Box>
<Text
className={`w-text cpwwn14 cb1oezn`}>
{"たいくんの部屋"}
</Text>
</Box>
</Link>
<Link
href={"https://x.com/i_am_taikun"}
target={"_blank"}
className={`w-link c14njgcc`}>
<Box
className={`w-box c16u96pk c1nrl0mm cv98wn5 cj7fjzz cjft9ro c10hsz cz9jktr ckhpsmk c1yh0sse c15p346f ckfbvdv cnny46r c1x20bz7 c1kiu74r c1omme4v c11v1l1o c1sy5cbg ctibe6j cq98dcv c1nljn1v cwnqwob`}>
<Box
className={`w-box cpspxpc cw8jop0 c1iiw5pw chpom5m c329hqk c1sy5cbg cxovp9q c2y4chl c1nljn1v`}>
<HtmlEmbed
code={"<svg xmlns=\"http://www.w3.org/2000/svg\" viewBox=\"0 0 512 512\"><!--!Font Awesome Free 6.7.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free Copyright 2025 Fonticons, Inc.--><path fill=\"#fff\" d=\"M389.2 48h70.6L305.6 224.2 487 464H345L233.7 318.6 106.5 464H35.8L200.7 275.5 26.8 48H172.4L272.9 180.9 389.2 48zM364.4 421.8h39.1L151.1 88h-42L364.4 421.8z\"/></svg>"}
className={`w-html-embed`} />
</Box>
<Text
className={`w-text cpwwn14 cb1oezn`}>
{"たいくん"}
</Text>
</Box>
</Link>
<Link
href={"https://www.youtube.com/@i_am_taikun"}
target={"_blank"}
className={`w-link c14njgcc`}>
<Box
className={`w-box c16u96pk c1nrl0mm cv98wn5 cj7fjzz cjft9ro c10hsz cz9jktr ckhpsmk c1yh0sse c15p346f ckfbvdv cnny46r c1x20bz7 c1kiu74r c1omme4v c11v1l1o c1sy5cbg ctibe6j cq98dcv c1nljn1v cwnqwob`}>
<Box
className={`w-box cpspxpc c1d3h5bc c1iiw5pw chpom5m c329hqk c1sy5cbg cxovp9q c2y4chl c1nljn1v`}>
<HtmlEmbed
code={"<svg xmlns=\"http://www.w3.org/2000/svg\" viewBox=\"0 0 576 512\"><!--!Font Awesome Free 6.7.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free Copyright 2025 Fonticons, Inc.--><path fill=\"#fff\" d=\"M549.7 124.1c-6.3-23.7-24.8-42.3-48.3-48.6C458.8 64 288 64 288 64S117.2 64 74.6 75.5c-23.5 6.3-42 24.9-48.3 48.6-11.4 42.9-11.4 132.3-11.4 132.3s0 89.4 11.4 132.3c6.3 23.7 24.8 41.5 48.3 47.8C117.2 448 288 448 288 448s170.8 0 213.4-11.5c23.5-6.3 42-24.2 48.3-47.8 11.4-42.9 11.4-132.3 11.4-132.3s0-89.4-11.4-132.3zm-317.5 213.5V175.2l142.7 81.2-142.7 81.2z\"/></svg>"}
className={`w-html-embed`} />
</Box>
<Text
className={`w-text cpwwn14 cb1oezn`}>
{"たいくん"}
</Text>
</Box>
</Link>
</Box>
</Box>
</Box>
</Box>
</Body>
}


      export { Page }
    